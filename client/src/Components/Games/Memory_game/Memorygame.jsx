...
import celebration from './jsonfile/celebrations-begin.json';
import './Memorygame.scss';

import background from './vid/memorybackground.mp4';
import dialogueSound from './sound/wining.wav';
import moveSoundFile from './sound/move-lock.wav';
// components
...
function Memorygame(props) {
  ...
  const calculateMerit = () => {
    ...
    const sound = new Audio(dialogueSound); // Create an Audio element
    const [isSoundPlaying, setIsSoundPlaying] = useState(false); // State to track if the sound is playing
    useEffect(() => {
      if (showCongrats && !isSoundPlaying) {
        sound.play(); // Play the sound effect
        setIsSoundPlaying(true);
      }
    }, [showCongrats, isSoundPlaying]);
  
    const resetGame = () => {
      setShowCongrats(false); // Close the dialogue box
      setIsSoundPlaying(false); // Reset the sound playing state
      sound.pause(); // Pause the sound effect
      sound.currentTime = 0; // Reset the sound effect to the beginning
      resetHandler(); // Reset the game
    };
    ...
    <>
      <div className="play-details">
        ...
        <div className="play-details-body memory-game">
          <div className="memorygame-heading">
          <h1 className="memorygame-title">MEMORY MATCH</h1>
      
          </div>
          <h2 className="guide" onClick={toggle}>
            <button className="guide-button">How to Play?</button>
          </h2>
            ...
            <div className="game-body">
              {newImgArray.map((item, idx) => {
                return (
                  ...
                  >
                    <div className={`memory-img-container ${item?.show ? 'shown' : 'hidden'}`}>
                      <img alt="img" className="item-img" src={item?.img} />
                    </div>
                  </div>
                );
              })}
              <div className="memorygame-footer">
                <div className="memorygame-footer-content">
                  <button className="button-footer-moves">Moves: {boardStats.moves}</button> 
                  <button className="button-footer-time">Time: {Math.floor(boardStats.elapsedTime)} s</button>
                </div>
      ...
      <div className="memory-background-video">
      <video autoPlay muted loop className="video-element"preload="auto">
        <source src={background} type="video/mp4" />
      </video>
    </div>
    {showCongrats && (
            <>
              <div className="congrats-dialogue">
                    <div className="lottie-background">
                    <Lottie animationData={celebration} className="lottie-celebration" />
                      </div>
                <h3 style={{fontWeight:'bold'}}>Congratulations!</h3>
                <p>
                  {calculateMerit()} performance in the Memory Game. Keep up the great work and
                  continue sharpening your memory skills!
                </p>
