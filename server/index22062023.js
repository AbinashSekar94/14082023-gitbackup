...
app.get("/status/filename", async (req, res) => {
  try {
    ...
    const folderName = `Status/${defaultValue}/`;
...
app.post("/upload", upload.single("pdfFile"), async (req, res) => {
  ...
  const containerClient = blobServiceClient.getContainerClient(containerName);
  const blobName = `${folderName}/${req.body.fileName}`;
...
app.post("/statusupload", upload.single("imageFile"), async (req, res) => {
  const containerName = "vetrikkodi";
  ...
  const dateObj = new Date(req.body.fileName.substring(0, 10)); // Extract the date from the file name
  ...
  const blobName = `${folderName}/${dateString}/${req.file.originalname}`; // Use the original image name
...
app.post("/categories", async (req, res) => {
  ...
  try {
    const connection = await pool.getConnection();
    const [rows, fields] = await connection.query(
      "SELECT DISTINCT category FROM quizfulldata WHERE level=?",
      [level]
    );
    connection.release();
    res.json(rows);
...
app.get("/levels", async (req, res) => {
  try {
    const connection = await pool.getConnection();
    const [rows, fields] = await connection.query(
      "SELECT DISTINCT level FROM quizfulldata"
    );
    connection.release();
...
app.post("/topic", async (req, res) => {
  ...
  try {
    const connection = await pool.getConnection();
    const [rows, fields] = await connection.query(
      "SELECT DISTINCT topic,category, topicid FROM quizfulldata WHERE category=? AND level=?",
      [category, level]
    );
    connection.release();
    res.json(rows);
...
app.post("/questions", async (req, res) => {
  ...
  try {
    const connection = await pool.getConnection();
    const [rows, fields] = await connection.query(
      "SELECT * FROM quizfulldata WHERE category=? AND level=? AND topic=?",
